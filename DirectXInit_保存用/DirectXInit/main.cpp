...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_  HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nCmdShow)
{
	...
	//ウィンドウのサイズを修正
	RECT rc1, rc2;
	GetWindowRect(hWnd, &rc1);//タイトルバーなどを含めた全体の寸法
	GetClientRect(hWnd, &rc2);//タイトルバーなどを含めない内側の寸法
	...
	//FPS測定用変数
	int fpsCounter = 0;
	long long oldTick = GetTickCount64();//前回測定時の時間
	long long nowTick = oldTick;//今回測定時の時間
...
int Move_g_key[4] = {0,0,0,0};//0が右、1が左、2が上、3が下
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN: //キー入力があったメッセージ
		...
		if (LOWORD(wParam) == VK_RIGHT)//ご飯をあげる
		{ //入力されたキーが右なら
			if(Move_g_key[0] == 0)
			{
				Move_g_key[0] = 1;
