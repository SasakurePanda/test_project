...
HRESULT D3D_Create(HWND hwnd)
{
	...
	CRect              rect;
	DXGI_SWAP_CHAIN_DESC swapChainDesc;//ここ分からん
...
{
	// 画面塗りつぶし色
	float clearColor[4] = { 0.0f, 0.0f, 1.0f, 1.0f }; //red,green,blue,alpha//ある

	// 描画先のキャンバスと使用する深度バッファを指定する
	g_pDeviceContext->OMSetRenderTargets(1, &g_pRenderTargetView, g_pDepthStencilView);//ある
	// 描画先キャンバスを塗りつぶす
	g_pDeviceContext->ClearRenderTargetView(g_pRenderTargetView, clearColor);//ある
	// 深度バッファをリセットする
...
void D3D_Release()
{
	if (g_pDeviceContext) g_pDeviceContext->ClearState();//ある
	...
	SAFE_RELEASE(g_pRenderTargetView);//ある
	SAFE_RELEASE(g_pSwapChain);//ある
	SAFE_RELEASE(g_pDeviceContext);
	SAFE_RELEASE(g_pDevice);//ある
	SAFE_RELEASE(g_pDeviceContext);//ある
}
